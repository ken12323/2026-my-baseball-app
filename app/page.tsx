...
export default async function Home({ searchParams }: { searchParams: { period?: Period, cat?: Category } }) {
  ...
  if (performance && players) {
    performance.forEach((perf) => {
      // player_id の型ズレを防ぐため String() に変換
      const player = players.find((p) => String(p.player_id) === String(perf.player_id));
      if (!player) return;

      let keys: string[] = [];
      if (category === 'high_school') keys.push(player.high_school || '経歴なし(外国人など)');
      else if (category === 'university') if (player.university) keys.push(player.university);
      ...
      } else if (category === 'draft_year') if (player.draft_year) keys.push(`${player.draft_year}年指名`);
      else if (category === 'hometown') keys.push(player.hometown || '不明');

      keys.forEach(key => {
        if (!key || key === '-' || key === '未設定') return;
        if (!stats[key]) stats[key] = { name: key, total_hits: 0, total_hr: 0, total_rbi: 0, players: [] };
        stats[key].total_hits += perf.h_hits;
        stats[key].total_hr += perf.h_hr;
        stats[key].total_rbi += perf.h_rbi;
        const pIdx = stats[key].players.findIndex(p => p.name === (player.player_name || player.name));
        if (pIdx === -1) {
          stats[key].players.push({ name: player.player_name || player.name, hits: perf.h_hits, hr: perf.h_hr, team: player.team_name });
        } else {
      ...
      <div className="max-w-4xl mx-auto">
        <header className="mb-6 bg-white p-6 rounded-2xl shadow-xl border-t-8 border-blue-900">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-right">
              <p className="text-[10px] font-bold text-blue-600 uppercase">Mode: {period} / {category}</p>
              <p className="text-[10px] text-slate-400">{ranking.length} groups found</p>
            </div>
          </div>
          ...
          <nav className="flex flex-wrap gap-1.5">
            {[
              ...
              <Link key={c.id} href={`/?period=${period}&cat=${c.id}`} className={`px-4 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${category === c.id ? 'bg-blue-900 text-white' : 'bg-slate-50 text-slate-400'}`}>
                {c.n}
              </Link>
            ))}
          </nav>
          ...
          <div className="mt-4 flex bg-slate-100 p-1 rounded-xl">
            {(['today', 'yesterday', 'weekly', 'season'] as Period[]).map(p => (
              <Link key={p} href={`/?period=${p}&cat=${category}`} className={`flex-1 text-center py-2 rounded-lg text-[10px] font-black uppercase ${period === p ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-400'}`}>
        ...
        <div className="space-y-3">
          {ranking.map((item, index) => (
            <details key={item.name} className="group bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <summary className="flex items-center justify-between p-5 cursor-pointer list-none">
                ...
                <div className="flex gap-6 items-center">
                  <div className="text-right">
                    <p className="text-[10px] text-slate-400 font-black">HITS</p>
                    <p className="text-2xl font-black text-blue-900">{item.total_hits}</p>
                  </div>
                  <div className="text-right border-l pl-6">
                    <p className="text-[10px] text-slate-400 font-black">HR</p>
                    <p className="text-2xl font-black text-red-600">{item.total_hr}</p>
                  </div>
                </div>
              </summary>
              <div className="px-16 pb-6 pt-2 bg-slate-50 border-t">
                <div className="flex flex-wrap gap-2">
                  {item.players.map(p => (
                    <div key={p.name} className="bg-white border p-2 rounded shadow-sm text-xs">
                      <span className="font-bold">{p.name}</span> <span className="text-blue-600">{p.hits}H</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {ranking.length === 0 && (
            <div className="text-center py-24 bg-white rounded-2xl border-2 border-dashed">
              <p className="text-slate-400 font-bold">データがありません（現在の日付：{targetDate}）</p>
              <p className="text-slate-300 text-xs mt-2">DBに対象日のデータがないか、選手IDが一致していません。</p>
            </div>
          )}
        </div>
      </div>
