...
const METRIC_INFO: Record<string, { desc: string, calc: string, benchmark: string }> = {
  ...
  roman: { desc: '打率は低いが、純粋な長打力（ISOp）があり、三振を恐れずフルスイングする「真のロマン砲」度合いを示す当サイト独自指標。', calc: 'ISOp + 三振率 - 打率', benchmark: '数値が高いほど「三振かホームランか、当たればデカいが確実性は低い」というファンが夢を見るロマン溢れる打者。' }
};
