...
// ※Step3でここをSupabaseからのデータに差し替えます
const routeData = [
  { route: '高卒', players: 223, hr: 4684, hits: 49980, avgHr: 21.0 },
  { route: '大卒', players: 157, hr: 2916, hits: 34918, avgHr: 18.6 },
  { route: '社会人・独立リーグ', players: 95, hr: 1240, hits: 22071, avgHr: 13.1 },
];
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-800">
      ...
      <div className="bg-blue-50 border-2 border-blue-400 rounded-xl p-4 mb-6 shadow-sm">
        <p className="text-xs md:text-sm font-bold text-blue-900 leading-relaxed">
          💡 データの真実：現役選手数では高卒が最多であり、1人あたりの平均本塁打数でも「21.0本」とトップを記録しています。スラッガー（長距離砲）を狙う場合のドラフト戦略において、高卒指名の重要性を裏付けるデータです。
        </p>
      </div>
        ...
        <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 shadow-md">
          ...
          <div className="space-y-4">
            {routeData.map((item, index) => {
              ...
              return (
                <div key={item.route} className="flex items-center justify-between border-2 border-slate-100 rounded-xl p-3 bg-slate-50">
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="bg-slate-800 text-white text-xs md:text-sm">
                  ...
                  <th className="p-3 font-bold text-right">合計本塁打</th>
                  <th className="p-3 font-bold text-right">合計安打</th>
              ...
              <tbody>
                {routeData.map((item) => (
                  <tr key={item.route} className="border-b-2 border-slate-100 hover:bg-slate-50 text-xs md:text-sm font-bold text-slate-700">
                    ...
                    <td className="p-3 text-right text-amber-600 font-extrabold">{item.hr.toLocaleString()}本</td>
                    <td className="p-3 text-right">{item.hits.toLocaleString()}安打</td>
