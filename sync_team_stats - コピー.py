...
BATTER_DB_COLS = ["player_id", "名前", "年度", "所属球団", "背番号", "試合", "打席", "打数", "得点", "安打", "二塁打", "三塁打", "本塁打", "塁打", "打点", "盗塁", "盗塁刺", "犠打", "犠飛", "四球", "死球", "三振", "併殺打", "打率", "長打率", "出塁率", "野手WAR", "wOBA", "OPS", "ISOp", "ランク"]
PITCHER_DB_COLS = ["player_id", "名前", "年度", "所属球団", "背番号", "登板", "勝利", "敗戦", "セーブ", "ホールド", "HP", "完投", "完封", "無四球", "打者", "投球回", "安打", "本塁打", "四球", "死球", "三振", "暴投", "ボーク", "失点", "自責点", "防御率", "投手WAR", "FIP", "WHIP", "K/9", "BB/9", "K/BB", "K-BB%", "ランク"]
...
def fetch_master_data():
    global MASTER_PLAYER_MAP
    print("🔄 選手マスター(playersテーブル)を取得中...")
    res = supabase.table("players").select("player_id, player_name").execute()
    for p in res.data:
        clean_name = re.sub(r'\s+', '', unicodedata.normalize('NFKC', p.get("player_name", "")))
        if clean_name not in MASTER_PLAYER_MAP:
            MASTER_PLAYER_MAP[clean_name] = str(p.get("player_id")).zfill(8)
    print(f"✅ マスターデータ取得完了 ({len(MASTER_PLAYER_MAP)}名)")
...
def scrape_team(team_name, team_id, mode="battingstats"):
    ...
    for table in tables:
        ...
        cols = [re.sub(r'\s+', '', unicodedata.normalize('NFKC', th.text)) for th in thead.find_all("th")]
        try: name_idx = cols.index("選手名")
        ...
        for row in rows:
            ...
            # ★核心：YahooIDではなく、マスターテーブルのIDを使用する
            if clean_name not in MASTER_PLAYER_MAP:
                continue # マスターにいない選手はスキップ
                
            safe_pid = MASTER_PLAYER_MAP[clean_name]
