...
def scrape_farm_profiles():
    ...
    # 登録されている全選手を取得（不足している情報を全て上書きして完全な状態にするため）
    res = supabase.table(table_name).select("player_id, sportsnavi_id, player_name").execute()
    ...
    # sportsnavi_idが登録されている選手のみ対象
    players = [p for p in res.data if p.get("sportsnavi_id")]
    
    if not players:
        print(f"✅ {table_name} に取得対象の選手はいません。")
        return

    print(f"🔍 全 {len(players)} 名のプロフィール情報を再構築します...")
    update_batch = []
    ...
    for p in players:
        ...
        try:
            ...
            # ★ 修正2：ポジションの抽出（メニューの「投手成績」等の誤爆を完全防止）
            for tag in soup.find_all(["p", "span", "div", "h1", "h2", "li"]):
                t = tag.text.strip()
                if len(t) <= 15 and "成績" not in t and "一覧" not in t:
            ...
            for raw_label, raw_val in pairs:
                ...
                if "出身地" in label and "hometown" not in profile_data:
                    ...
                    # ★ 修正3：育成ドラフトの厳格な判定フラグ
                    profile_data["is_developmental"] = "育成" in val
                elif "経歴" in label and "high_school" not in profile_data:
                    # ★ 修正4：カタカナに続く長音記号（スポーツ等）を破壊せず、安全に経歴を分割
                    safe_val = re.sub(r'[－\-]', '|', val)  # ハイフンはパイプに変換
                    safe_val = re.sub(r'ー(?=[^ァ-ンヴー])', '|', safe_val) # カタカナ以外の前にある長音記号だけパイプに変換
                    
                    history = [h.strip() for h in safe_val.split('|') if h.strip()]
                    ...
                    if len(history) > 4: profile_data["prev_team_3"] = history[4]
            
            update_batch.append(profile_data)
            pos = profile_data.get('position_detail', '不明')
            is_dev = "育成" if profile_data.get('is_developmental') else "支配下"
            print(f"取得完了: {p['player_name']} [{pos} / {is_dev}]")
            time.sleep(0.5) # サーバー負荷軽減（Yahooに怒られないように）
    ...
    if update_batch:
        ...
        print(f"🎉 {len(update_batch)} 名のプロフィール更新が完全に完了しました！")
